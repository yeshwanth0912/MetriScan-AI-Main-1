...
export async function exportReportAsPdf({ inspection, results, user = null }) {
  ...
  doc.text('REPORT / REFERENCE ID', col1, y + 4.5);
  doc.text('INSPECTION TIMESTAMP', col2, y + 4.5);
  doc.text('INSPECTING OFFICER', col3, y + 4.5);
  doc.text('INSPECTION CHANNEL', col4, y + 4.5);

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(15, 23, 42);
  doc.text(inspection.reference || inspection.id || 'N/A', col1, y + 9);
  doc.text(formatTimestamp(inspection.analysed_at || inspection.started_at), col2, y + 9);
  doc.text(currentUser.name || currentUser.username || 'Authorized Officer', col3, y + 9);
  doc.text((inspection.channel || 'retail').toUpperCase(), col4, y + 9);

  // Row 2
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.5);
  doc.setTextColor(100, 116, 139);
  doc.text('PRODUCT / BRAND', col1, y + 14);
  doc.text('COMMODITY NAME', col2, y + 14);
  doc.text('PREMISES / ESTABLISHMENT', col3, y + 14);
  doc.text('BARCODE / GTIN', col4, y + 14);

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(15, 23, 42);
  const brandProd = `${inspection.product?.brand || ''} ${inspection.product?.product_name || ''}`.trim() || 'Packaged Commodity';
  doc.text(brandProd.length > 25 ? brandProd.substring(0, 23) + '…' : brandProd, col1, y + 18.5);

  const commName = inspection.product?.product_name || 'Standard Packaged Commodity';
  doc.text(commName.length > 25 ? commName.substring(0, 23) + '…' : commName, col2, y + 18.5);

  const premisesStr = `${inspection.premises || 'Commercial / Retail Premises'}`;
  doc.text(premisesStr.length > 23 ? premisesStr.substring(0, 21) + '…' : premisesStr, col3, y + 18.5);

  doc.text(inspection.product?.barcode || 'N/A', col4, y + 18.5);
  ...
  // Confidence & Metric Sub-Boxes
  // Box 1: Adjusted Confidence
  doc.setFillColor(255, 255, 255);
  doc.setDrawColor(226, 232, 240);
  doc.roundedRect(margin + 5, y + 16, 40, 13, 1.5, 1.5, 'FD');
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6);
  doc.setTextColor(100, 116, 139);
  doc.text('ADJUSTED CONFIDENCE', margin + 8, y + 20);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(...(adjustedConf >= 75 ? [22, 101, 52] : adjustedConf >= 50 ? [180, 83, 9] : [185, 28, 28]));
  doc.text(`${adjustedConf}%`, margin + 8, y + 26.5);

  // Box 2: Pass Count
  doc.roundedRect(margin + 48, y + 16, 28, 13, 1.5, 1.5, 'FD');
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6);
  doc.setTextColor(100, 116, 139);
  doc.text('RULES PASSED', margin + 51, y + 20);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(22, 101, 52);
  doc.text(`${passedRules.length}`, margin + 51, y + 26.5);

  // Box 3: Failures Count
  doc.roundedRect(margin + 79, y + 16, 28, 13, 1.5, 1.5, 'FD');
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6);
  doc.setTextColor(100, 116, 139);
  doc.text('VIOLATIONS', margin + 82, y + 20);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(failedRules.length > 0 ? 185 : 100, failedRules.length > 0 ? 28 : 116, failedRules.length > 0 ? 28 : 139);
  doc.text(`${failedRules.length}`, margin + 82, y + 26.5);

  // Box 4: Review Needed
  doc.roundedRect(margin + 110, y + 16, 28, 13, 1.5, 1.5, 'FD');
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6);
  doc.setTextColor(100, 116, 139);
  doc.text('REVIEW NEEDED', margin + 113, y + 20);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(reviewRules.length > 0 ? 180 : 100, reviewRules.length > 0 ? 83 : 116, reviewRules.length > 0 ? 9 : 139);
  doc.text(`${reviewRules.length}`, margin + 113, y + 26.5);

  // Box 5: Optical Quality Factor
  doc.roundedRect(margin + 141, y + 16, 36, 13, 1.5, 1.5, 'FD');
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6);
  doc.setTextColor(100, 116, 139);
  doc.text('OPTICAL CLARITY', margin + 144, y + 20);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  doc.text(`${quality.score}/100 (${quality.status})`, margin + 144, y + 26.5);
  ...
  if (failedRules.length > 0) {
    ...
    doc.text(`STATUTORY VIOLATION AUDIT: GROUNDS FOR NON-COMPLIANCE (${failedRules.length} DEFECTS)`, margin + 4, y + 4.8);
    ...
    failedRules.forEach((fail, idx) => {
      ...
      const boxHeight = 16 + (reasonLines.length * 3.5);
      ...
      doc.text(`[${fail.rule_code || 'LMPC-RULE'}] ${fail.title || 'Mandatory Declaration Violation'}`, margin + 5, y + 4.5);

      // Severity / Tag
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(6.5);
      doc.setTextColor(185, 28, 28);
      doc.text('FAILED — NON-COMPLIANT', margin + contentWidth - 36, y + 4.5);
      ...
      doc.text(legalRef, margin + 5, y + 8.5);

      // Exact Failure Reason
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7.2);
      doc.setTextColor(30, 41, 59); // slate-800
      doc.text(reasonLines, margin + 5, y + 13);

      // Legal Consequences & Remedy
      const remedyY = y + 13 + (reasonLines.length * 3.5);
  ...
  if (reviewRules.length > 0) {
    ...
    doc.text(`FINDINGS REQUIRING OFFICER CONFIRMATION (${reviewRules.length} ITEMS)`, margin + 4, y + 4.8);
    ...
    reviewRules.forEach((rev) => {
      ...
      const boxHeight = 12 + (revLines.length * 3.5);
      ...
      doc.text(`[${rev.rule_code || 'LMPC-RULE'}] ${rev.title || 'Verification Review'}`, margin + 5, y + 4.5);

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(6.5);
      doc.setTextColor(180, 83, 9);
      doc.text('REVIEW REQUIRED', margin + contentWidth - 28, y + 4.5);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7);
      doc.setTextColor(71, 85, 105);
      doc.text(revLines, margin + 5, y + 9);
  ...
  doc.text('DECLARED PACK VALUE / EXTRACTED TEXT', margin + 48, y + 4.2);
  doc.text('PANEL', margin + 120, y + 4.2);
  doc.text('HEIGHT', margin + 138, y + 4.2);
  doc.text('STATUS', margin + 158, y + 4.2);
  ...
  allRuleResults.forEach((r) => {
    ...
    doc.setFillColor(
      isPass ? 240 : isFail ? 254 : isReview ? 254 : 248,
      isPass ? 253 : isFail ? 242 : isReview ? 243 : 250,
      isPass ? 244 : isFail ? 242 : isReview ? 199 : 252
    );
    doc.setDrawColor(
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(15, 23, 42);
    doc.text(`${r.rule_code || 'LMPC'}: ${r.title || 'Rule Assessment'}`, margin + 3, y + 4.2);

    // Badge
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
    doc.setTextColor(
      isPass ? 22 : isFail ? 185 : isReview ? 146 : 100,
      isPass ? 101 : isFail ? 28 : isReview ? 64 : 116,
      isPass ? 52 : isFail ? 28 : isReview ? 14 : 139
    );
    doc.text(r.result || 'PASS', margin + contentWidth - 22, y + 4.2);
    ...
    doc.text(doc.splitTextToSize(reasonText, contentWidth - 26), margin + 3, y + 8);
