...
<head>
  ...
  <style>
    ...
    h1 { margin-bottom: 4px; font-size: 24px; color: #0F172A; font-weight: 700; }
    h3 { margin-top: 24px; margin-bottom: 8px; font-size: 16px; color: #0F172A; }
    .status { display: inline-block; padding: 6px 14px; font-weight: bold; border-radius: 4px; color: #FFFFFF; background: ${foundInspection.compliance_status === 'COMPLIANT' ? '#15803D' : foundInspection.compliance_status === 'NON_COMPLIANT' ? '#DC2626' : '#D97706'}; }
...
<body>
  ...
  <p style="color: #475569; font-size: 13px;">Legal Metrology (Packaged Commodities) Rules, 2011 as amended &bull; Report: ${foundReport.reference}</p>
  ...
  <table>
    ...
    <tbody>
      ${foundInspection.fields.map(f => `<tr><td><strong>${f.field_name}</strong></td><td>${f.effective_value || 'Absent'}</td><td>${f.present ? 'Present' : 'Missing'}</td><td>${Math.round(f.confidence * 100)}%</td><td>${f.panel}</td></tr>`).join('')}
    </tbody>
  </table>
  ...
  <table>
    ...
    <tbody>
      ${foundInspection.rule_results.map(r => `<tr><td><strong>${r.rule_code}</strong></td><td>v${r.rule_version || '1.0'}</td><td><strong style="color: ${r.result === 'PASS' ? '#15803D' : r.result === 'FAIL' ? '#DC2626' : '#D97706'}">${r.result}</strong></td><td style="font-size: 12px; color: #334155; font-weight: 500;">${r.source_reference || 'LMPC Rules, 2011'}</td><td>${r.reason}</td></tr>`).join('')}
    </tbody>
  </table>
  <p style="font-size: 12px; color: #475569; margin-top: 30px;">Generated by MetriScan AI on ${new Date().toUTCString()}. This document reflects machine-assisted extraction verified by an authorized inspector.</p>
</body>
</html>`;
